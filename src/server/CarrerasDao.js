...
function CarrerasDao() {}

CarrerasDao.prototype.createSelectSql = function(rowGroupCols, valueCols, groupKeys) {
...
CarrerasDao.prototype.createFilterSql = function(key, item) {
...
CarrerasDao.prototype.createNumberFilterSql = function(key, item) {
    switch (item.type) {
        case 'equals':
...
CarrerasDao.prototype.createTextFilterSql = function(key, item) {
    switch (item.type) {
        case 'equals':
            return key + ' = "'+ item.filter +'"';
        case 'notEqual':
...
CarrerasDao.prototype.createWhereSql = function(rowGroupCols, groupKeys, filterModel) {
    var that = this;
    var whereParts = [];
...
CarrerasDao.prototype.createGroupBySql = function(rowGroupCols, groupKeys) {
    if (this.isDoingGrouping(rowGroupCols, groupKeys)) {
        var colsToGroupBy = [];
...
CarrerasDao.prototype.createOrderBySql = function(sortModel) {
    var sortParts = [];
    if (sortModel) {
...
CarrerasDao.prototype.isDoingGrouping = function(rowGroupCols, groupKeys) {
...
CarrerasDao.prototype.createLimitSql = function(startRow, pageSize) {
    return ' limit ' + (pageSize + 1) + ' offset ' + startRow;
};

CarrerasDao.prototype.getRowCount = function(startRow, pageSize, results) {
...
CarrerasDao.prototype.cutResultsToPageSize = function(pageSize, results) {
...
CarrerasDao.prototype.list = function(request, resultsCallback) {
...
module.exports = new CarrerasDao();
